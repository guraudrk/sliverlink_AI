...
export function deriveNextSteps(category: RagQueryCategory, evidence: RagEvidence[]): string[] {
  const nextSteps: string[] = [];
  if (evidence.some((item) => item.safetyFlags.includes("help_requested"))) {
    nextSteps.push("도움 요청한 항목 직접 확인하기");
  }
  if (category === "medication" && evidence.some((item) => item.safetyFlags.includes("medication_related"))) {
    nextSteps.push("복약 메모를 다시 확인하기");
  }
  return nextSteps;
}
