...
import { SocialScoreList } from "@/components/social/social-score-list";
...
export default async function DashboardSocialPage() {
  ...
  const listItems = parents.map((p, i) => ({
    id: p.id,
    display_name: p.display_name ?? "이름 없음",
    scores: scoresByParent[i],
    latestScore: scoresByParent[i][0]?.score ?? null,
  }));
    ...
    <div className="flex flex-1 flex-col items-center bg-slate-50 px-4 py-10 sm:py-16">
      ...
      {parents.length === 0 ? (
        ...
        <div className="mx-auto w-full max-w-2xl space-y-4">
          ...
          <SocialScoreList items={listItems} />
