...
export default async function ProtectedLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div style={{ display: "flex", minHeight: "100vh", backgroundColor: "var(--sl-bg)" }}>
      <NavigationProgress />
      ...
      >
        {/* Logo */}
        <div style={{ height: 64, display: "flex", alignItems: "center", padding: "0 20px", borderBottom: "1px solid var(--sl-border)", flexShrink: 0 }}>
          <Link href="/dashboard" style={{ display: "flex", alignItems: "center", gap: 10, textDecoration: "none" }}>
            ...
            <span style={{ fontSize: 15, fontWeight: 700, color: "var(--sl-ink)" }}>SilverLink AI</span>
          </Link>
        </div>
      ...
      <div className="md:ml-64" style={{ flex: 1, display: "flex", flexDirection: "column", minWidth: 0 }}>
        ...
        >
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
            ...
            >
              <Bell size={16} color="var(--sl-body)" strokeWidth={1.8} />
              {alertCount > 0 && (
                <span style={{ position: "absolute", top: 7, right: 7, width: 7, height: 7, borderRadius: "50%", backgroundColor: "#F04438", border: "1.5px solid var(--sl-card)" }} />
              )}
            </Link>

            {/* Avatar + name */}
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
              <div style={{ width: 32, height: 32, borderRadius: "50%", backgroundColor: "var(--sl-primary-tint)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 13, fontWeight: 700, color: "#2E5BFF", flexShrink: 0 }}>
                {emailPrefix.charAt(0).toUpperCase()}
              </div>
              <div>
                <p style={{ fontSize: 13, fontWeight: 600, color: "var(--sl-ink)", margin: 0, lineHeight: 1.2 }}>{emailPrefix}</p>
                <p style={{ fontSize: 11, color: "var(--sl-placeholder)", margin: 0 }}>{role === "caseworker" ? "복지사" : "가족"}</p>
              </div>
            </div>
          </div>
        </header>
