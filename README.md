...
## 10. 아직 구현하지 않은 기능 (2026-06-27 기준 최신)

Day15까지 완료한 뒤, 사용자가 정한 다음 순서대로 남은 항목을 진행할 예정입니다 (난이도/외부 승인 의존도를 감안해 재정렬됨):

1. **Resend 도메인 인증** — 현재 Resend는 샌드박스 모드로, 계정 소유자 본인 이메일에만 발송 가능(naver.com/Gmail+alias 모두 차단 확인됨). 커스텀 도메인 인증이 끝나야 임의의 가입자에게 정상적으로 인증 메일이 발송됨.
2. **웹페이지 대대적인 개편 + 배포 + 모바일 최적화** — 현재는 로컬 개발 서버 기준으로만 검증됨. 실제 배포 환경(Vercel 등) 및 반응형/모바일 사용성 점검 필요.
3. **실제 통화/SMS/카카오톡 발송 기능** — Day8의 `MockDeliveryProvider`와 Day14 챗봇의 "지금 알려드리기" 버튼은 모두 실제 네트워크 호출이 없는 Mock 상태. 실제 발송으로 전환하려면 Provider 선정(Twilio/카카오 비즈니스 메시지 등) 및 실비용 발생에 대한 별도 승인이 필요해 가장 마지막 순서로 미뤄둠.

그 외 자잘한 항목:
- `due_task_checker` 자동 알림 시나리오 (기한이 다가온 케어 업무를 감지해 알림을 큐에 자동으로 넣는 배치/크론)
- 회원 탈퇴, 비밀번호 재설정 등 계정 관리 기능
- RAG 비서의 답변 품질을 늘려가는 평가 케이스 확충(현재 `rag-evaluation.eval.ts`의 품질 케이스 + 보안 케이스 2종)
