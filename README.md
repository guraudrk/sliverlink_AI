...
## 10. 아직 구현하지 않은 기능 (2026-07-07 Day24 기준 최신)

Day24까지 완료한 뒤 남은 항목:

**단기 (Day25 — 실제 통화 end-to-end 테스트)**
1. **실제 Solapi 음성 전화 검증** — 배포된 Vercel 앱에서 실제 발신, 폰 수신 + 응답 키패드 + `delivery_attempts` 기록 확인. PRD: [`docs/PRD-day25-real-call-test.md`](docs/PRD-day25-real-call-test.md).
2. **통화 후 브리핑 실 데이터 검증** — 실제 통화 완료 후 `call_family_briefs`에 브리핑이 자동 생성되는지 확인.
3. **`social_scores` DB 마이그레이션 실행** — `docs/social-scores-migration.sql`을 Supabase SQL Editor에서 실행해야 Day23 기능이 실제 작동.

**중기 (Day26 — 공공기관 최적화)**
4. **공공기관 한눈에 보기 페이지** — 지자체/복지관 담당자가 여러 어르신 상태를 한 화면에서 모니터링할 수 있는 개요 페이지.
5. **KakaoTalk 알림톡 실사용** — `SolapiKakaoProvider` 코드 준비 완료. 카카오 비즈니스 채널 심사 통과 후 `ENABLE_REAL_KAKAO=true` 플래그 설정으로 활성화.

**Post-MVP**
6. **어르신 동의 상태(`consent_status`) 관리** — 실제 부모님 번호로 발신하기 전 동의 수집 UI/흐름.
7. **Resend 도메인 인증** — 현재 인증 메일이 계정 소유자에게만 발송됨. 실제 도메인 구매 후 DNS 인증 필요.
8. **쌍방향 AI 음성 에이전트** — OpenAI Realtime / Vapi / Retell 기반 자유대화형 전화. Post-MVP 실험 트랙.
