...
- [x] DRY_RUN 모드에서 폼 제출 → API → make-client까지 전체 플로우 정상 동작
- [x] `sender_name` / `target_person` / `message` 필수 검증 및 에러 처리 확인
- [x] 서버가 `source_channel="web"`, `requested_at`, `today_date`를 정확히 생성해 payload에 포함하는지 확인
- [x] `MAKE_WEBHOOK_URL`이 클라이언트에 노출되지 않음을 확인 (번들/네트워크 탭 점검)
- [x] Vitest 유닛 테스트(schema, payload) 통과
- [x] Playwright E2E 테스트(create-task) 통과
- [x] DRY_RUN=false로 실제 연동 확인 — Make 시나리오 트리거 → GPT 파싱 → Airtable `care_tasks`/`message_logs` 기록까지 동작 확인 (Make.com 시나리오 자체는 이 저장소 범위 밖)

## 11. 리스크 / 오픈 이슈
1. ~~Next.js 16의 실제 breaking change 범위 미확인~~ — **해결**: Route Handler 관련 공식 docs 확인 후 구현, 영향 없었음.
2. ~~Make.com 시나리오가 기대하는 정확한 payload 키 이름/형식이 이 PRD의 가정과 다를 수 있음~~ — **해결**: 실제 연동 확인 결과 GPT 파싱 → Airtable 기록까지 정상 동작.
3. ~~Zod/Vitest/Playwright가 아직 직접 의존성으로 설치되어 있지 않음~~ — **해결**: 모두 설치 및 테스트 작성 완료.
4. 자녀/관리자 역할 구분이 향후 필요해질 경우 입력 필드 또는 인증 체계 확장 필요 (현재는 동일 폼 공유) — **오픈 상태 유지**.
...
- 자녀/관리자용 웹 입력창 구현
- 자연어 돌봄 요청 입력
- Next.js API Route를 통한 서버 사이드 Webhook 호출
- Make Webhook 연동
- GPT 기반 요청 파싱 자동화 연결
- Airtable `care_tasks` 생성 확인
- `message_logs` inbound 기록 확인
- Dry Run 모드 구현
- `source_channel = web` 기반 멀티채널 확장 구조 반영

## 다음 단계로 분리한 범위

- `due_task_checker` 자동 알림 탐지
- `message_logs` outbound 기록
- `parent_notified` 업데이트
- 카카오톡 알림 발송
- 부모님 "완료" 응답 처리
